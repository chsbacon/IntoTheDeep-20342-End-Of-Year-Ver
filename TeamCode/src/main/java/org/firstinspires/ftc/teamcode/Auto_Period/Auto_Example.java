...
public class Auto_Example extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        */
        ...
        rotate(880,"clockwise");
        movement(3000,"up");
        moveArm(3090,"fuckOff");
        moveArm(3090,"shit");
    }

    // Method for diagonal movement
    public void moveArm(double temp, String temp1){
        double target=timer.time()+temp;
        if(temp1.equals("shit")){
            armServo1.setPower(1.0);

            armServo2.setPower(-1.0);
        }else{
            armServo1.setPower(-1.0);
        ...
        while(timer.time()<target) {
            telemetry.addData("fuck off", "time" + timer.time());
            telemetry.update();
        }
    }
